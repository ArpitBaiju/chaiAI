...
from layers.ela import compute_ela_score
from layers.metadata import compute_metadata_score
from layers.forensics.analyze import analyze_forensics
...
def analyze_image(image_path):
    ...
    #  Added forensic manipulation layer
    forensics_result = analyze_forensics(image_path)
...
def llm_decision_layer(image_path, layer_outputs):
    ...
    manipulation_score = layer_outputs["forensics_analysis"]["manipulation_score"]
    tamper_regions = layer_outputs["forensics_analysis"]["tamper_regions"]
    phash = layer_outputs["forensics_analysis"]["phash"]

    prompt = f"""
You are a professional digital image forensic analyst.

You have received reports from 5 independent forensic layers.
...
Classical Manipulation Forensics:
- Manipulation Probability: {manipulation_score}
...
{{
  ...
  "type": {{
      "ai_generated": float_between_0_and_1,
      "manipulation_score": float_between_0_and_1
  }},
  "media": {{
      "uri": "{os.path.basename(image_path)}"
  }},
  "forensic_summary": "short technical reasoning explaining the verdict",
  "Final Result": "AI Generated / AI Edited / Real"
}} Populate manipulation_score directly from the classical manipulation probability.
...
"""
    ...
    completion = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a strict forensic classifier. Output only JSON."
            },
            {
                "role": "user",
                "content": [
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("===== FINAL FORENSIC VERDICT =====\n")
        print("AI Generated Score :", parsed["type"]["ai_generated"])
        print("Final Result       :", parsed["Final Result"])
        print("Reasoning          :", parsed["forensic_summary"])
        print("\nFull JSON Output:\n")
